...
public class EZParallax : MonoBehaviour
{
	...
	public void InitializeParallax()
	{
		if(m_playerObj == null)
		{
			Debug.LogError("EZParallax initialized, but a player has not been assigned. Aborting.");
			return;
		}
		
		if(m_mainCamera == null)
		{
			Debug.LogError("EZParallax initialized, but a camera has not been assigned. Aborting.");
			return;
		}
		...
		{
			Debug.LogWarning("EZParallax initialized, but no objects have been assigned! No parallaxing effects will be present.");
			return;
		}
		...
		for(int i = 0; i < m_parallaxElements.Length; i++)
		{
			if(m_parallaxElements[i].parallaxObject)
			{
				...
				{
					Debug.LogError("EZParallax initialized, but a parallaxing object, " + m_parallaxElements[i].name + ", has the same Z axis position as the player. Move this object to a different z depth to allow EZ Parallax to work properly. Aborting.");
					return;
				}
			}
		}
	...
	void SpawnSingleElementDupes(EZParallaxObjectElement targetElement, Camera actualCamera, float screenWidth)
	{
		...
		if (targetElement.randomSpawnX)
		{
			...
			{
				Debug.LogWarning ("WARNING -- For your " + targetElement.name + " element, your minimum random spawn distance is greater than your maximum random spawn distance. Automatically swapping your minimum for your maximum.");
			...
			{
				Debug.LogError ("An EZParallax element object named " + targetElement.name + " needs to spawn more objects than there are slots in the random offset history! Raise your history size to greater than " + maxDupes + " to resolve this problem. Aborting the creation of duplicate objects for " + targetElement.name + ".");
				return;
			}
		}
		else
		...
		for ( int j = 0; j < numLeftDupes; ++j )
		{
			...
			{
				int offsetIndex = m_rndDistStartIndex - 1 - j;
			...
			{	
				int offsetIndex = m_rndDistStartIndex - 1 - j;
				leftDupeObject.position = new Vector3(leftDupeObject.position.x, targetElement.altSpawnAnchorY + (float)m_rndAltDistArray[dupeTargetElement.spawnGroupIndex, offsetIndex], leftDupeObject.position.z);
			}
			...
			dupeTargetElement.useRandomYSpawnAltOffset = targetElement.useRandomYSpawnAltOffset;
			dupeTargetElement.randomYSpawnLowestAltOffset = targetElement.randomYSpawnLowestAltOffset;
			dupeTargetElement.randomYSpawnHighestAltOffset = targetElement.randomYSpawnHighestAltOffset;
			dupeTargetElement.altSpawnAnchorY = targetElement.altSpawnAnchorY;
		...
		for ( int k = 0; k < numRightDupes; ++k )
		{
			...
			if(dupeTargetElement.randomSpawnX)
			{
				objectOffsetVector = new Vector3(randomOffsetsList[offsetListShiftIdx] + dupeTargetElement.meshWidth, 0, 0);
				dupeTargetElement.initialRightSpawnDistanceX = randomOffsetsList[offsetListShiftIdx];
				dupeTargetElement.initialRightSpawnDistanceScreenSpaceX = targetCenterPosScreenPt.x - actualCamera.camera.WorldToScreenPoint(dupeTargetElement.parallaxObject.position - (new Vector3(randomOffsetsList[offsetListShiftIdx], 0, dupeTargetElement.parallaxObject.position.z))).x;
			}
			else
			{
				objectOffsetVector = new Vector3(targetElement.spawnDistanceX + dupeTargetElement.meshWidth, 0, 0);
			...
			if(dupeTargetElement.useRandomYSpawnAltOffset)
			{	
				rightDupeObject.position = new Vector3(rightDupeObject.position.x, targetElement.altSpawnAnchorY + (float)m_rndAltDistArray[dupeTargetElement.spawnGroupIndex, offsetListShiftIdx + 1], rightDupeObject.position.z); //+1 to compensate for offset vs actual positions in array, skipping the starting object
			}
	...
	{
		numRightDupes = Mathf.Min( Mathf.CeilToInt( ( (screenWidth * 1.5f) - targetCenterPosScreenPt.x) / targetElementScreenWidth), maxDupes);
		if(numRightDupes < 0)
		{
			numRightDupes = 0;
		}
	...
	private EZParallaxObjectElement FindEdgeSpawningElement( EZParallaxObjectElement targetElement, bool direction)
	{
		if(direction) //If moving to the right
		{
			if(targetElement.dupeElementLeft == null || targetElement.dupeElementLeft == targetElement)
			{
				...
				{
					Debug.LogWarning("WARNING -- You are trying to manipulate an element, " + targetElement.name + ", that was set to spawn duplicates, but in fact has none. You may need to initialize, or may have purged your duplicates without respawning new ones.");
				}
	...
	void LateUpdate()
	{
		...
		if(camFrameDelta !=  Vector3.zero )
		{
			for(int i = 0; i < m_parallaxElements.Length; i++)
			{
				...
				{
					Debug.LogWarning ("WARNING -- There is a parallax object, named " + targetPE.name + ", in your the EZ Parallax element list that doesn't have an actual transform attached to it. This is impossible to achieve from the EZP inspector, so your code may be causing this issue at runtime.");
					continue;
				}
	...
	void UpdateDupeObjects(EZParallaxObjectElement targetPE, bool direction)
	{
		...
		float newSpawnOffsetScreenSpace = 0.0f;
		...
		{
			Debug.LogWarning("WARNING -- UpdateDupeObjects is trying to access a parallax object, for the EZP object element " + edgeElement.name + ", that doesn't exist! Object may have been destroyed during runtime. objectPosX is remaining 0.");
		}
		...
		if(direction) //Element is moving to the left side of the screen. Camera most likely is moving to the right.
		{
			while( (objectPosX + targetPE.elementScreenSpaceExtentX + edgeCheckBuffer) < 0.0f )
			{
				if(targetPE.randomSpawnX)
				{
					...
					Vector3 objectOffsetVector = new Vector3(newSpawnOffset, 0.0f, 0.0f);
	...
	public void AssignPlayer(GameObject targetPlayerObj, bool doInit)
	{
		...
		{
			Debug.LogError("EZ Parallax AssignPlayer was passed a null object. Aborting function. Player was not assigned.");
			return;
		}
	...
	public void AssignCamera(GameObject targetCameraObj, bool doInit)
	{
		...
		{
			Debug.LogError("EZ Parallax AssignCamera was passed a null object. Aborting function. Camera was not assigned.");
			return;
		}
	...
	public EZParallaxObjectElement AddNewParallaxingElement(Transform targetElement, float privateSpeedScalarX, float privateSpeedScalarY)
	{
		for(int i = 0; i < m_parallaxElements.Length; i++)
		{
			...
			{
				Debug.LogWarning("WARNING -- AddNewParallaxingElement attempted to add an element that was arleady in the parallax list. Aborting.");
				return null;
			}
		}
		List<EZParallaxObjectElement> tempElementArray = new List<EZParallaxObjectElement>(m_parallaxElements);
	...
	public void PurgeSingleDupeChain(Transform targetTransform)
	{
		...
		{
			Debug.LogWarning("The call to PurgeSingleDupeChain could not find the target transform. Aborting purge.");
			return;
		}
		...
		if(targetElement.dupeElementRight != null)
		{
			...
			//Get all elements to the right of our target element in the chain.
